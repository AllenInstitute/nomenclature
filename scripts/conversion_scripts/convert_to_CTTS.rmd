...
First, let's define the sample.csv.  Note that Uri's for each sample are required, but that we can use fake ones until real ones exist.  Also note that preferred aliases need to be unique, so we will set the preferred alias as part of the cell set accession ID in combination with the existing preferred alias or (if missing) the lowest ancestor's preferred alias.
...
fwrite(cellSetFamily,"CellSetToCellSet.csv")
```
...
fwrite(cellSetAliases,"CellSetToCellTypeAlias.csv")
```    
...
```{r}
create_zip_archive("mouse_m1_miniatlas.zip")  # Optional filename
```
